...
public class generateRoguelike extends generator{
  ...
  */
	public Vector2[] chooseRandomExit(Room room){
		Vector2[] posangle = {new Vector2(0,0),new Vector2(0,0)};
		
		switch(rand.rand_choice(4)) {
			case 0:
				posangle[0].x=room.minx;
				posangle[0].y=rand.rand_range(room.miny+1, room.maxy-1);
				posangle[1].x=-1;
				posangle[1].y=0;
				break;
			case 1:
				...
				break;
			case 2:
				posangle[0].x=rand.rand_range(room.minx+1, room.maxx-1);
  ...
  */
	...
	boolean drawCorridor(Vector2[] s, Vector2[] e) {
		boolean success;
		success=findPath(new Vector2(s).toArr(),new Vector2(e).toArr());
		// Draw our doors.  We only do this if our path creation was successful.
		if(success) {
			System.out.println("Doors");
			setPathTile(s[0].x,s[0].y);
			setPathTile(e[0].x,e[0].y);
		}
		return success;
	}
    ...
    // If we fail to connect 40 times, we declare this dungeon sucky and go again...
		...
		for(i=0;i<rooms.size()-1;i++) {
			...
			Vector2[] vecs = chooseRandomExit(rooms.get(src));
			Vector2[] vecs2 = chooseRandomExit(rooms.get(i+1));
